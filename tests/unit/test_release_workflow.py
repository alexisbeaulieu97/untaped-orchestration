...
PER-TOOL CONFIG block. Everything below the closing divider must stay byte-identical to
...
INTERNAL_DEPS: list[tuple[str, str | None]] = [
    ("untaped>=3.1.0,<4", None),
]
# Docs that must steer users to PyPI install (repo-relative paths); [] to skip the docs check.
PYPI_INSTALL_DOCS: list[str] = [
    "README.md",  # REPLACE list
]
# REPLACE — reviewed, merged 40-character core commit SHA containing the shared checker.
CORE_RELEASE_TOOL_SHA = "80bb8411cd0017f3e0cde818656aaf6fd0233368"
# ========================================================================
...
def test_core_release_tool_sha_validator_rejects_mutable_or_malformed_refs() -> None:
    untouched_sentinel = "__CHECKER_" + "SHA__"
    invalid_refs = [
        "main",
        "v3.1.0",
        untouched_sentinel,
    ...
    assert not any(_is_immutable_core_sha(ref) for ref in invalid_refs)
    assert _is_immutable_core_sha("a" * 40)
...
def test_release_tooling_is_sourced_from_pinned_core_checkout() -> None:
    ...
    assert _is_immutable_core_sha(CORE_RELEASE_TOOL_SHA), (
        "CORE_RELEASE_TOOL_SHA must be a reviewed, merged 40-character lowercase core commit SHA"
    )
